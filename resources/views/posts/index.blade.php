             ...
             <article class="w-full h-80 bg-cover bg-center @if($loop->first) md:col-span-2  @endif " style="background-image: url('{{ Storage::url($post->image->url)}}'); ">
               <div class="w-full h-full px-8 flex flex-col justify-center">
                <div class="">
                  ...
                  <a href="{{route('post.tag',$tag)}}" class="inline-block px-3 h6 bg-{{$tag->color}}-600 text-white rounded-full">{{$tag->name}}</a>
                  @endforeach
                </div>
                 <h1 class="text-6xl text-white leading-8 font-bold mt-2">
